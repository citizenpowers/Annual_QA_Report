...
summarise(`Number of Records`=n(),`Percent of Total`=percent(n()/nrow(DBHYDRO_data_2021),accuracy = 0.01)) %>% filter(FLAG=="yes")  %>% arrange(desc(`Number of Records`)) %>% ungroup %>% slice_head(n=25) %>% kable() %>% kable_styling(bootstrap_options = c("striped", "hover")) 
```
###Distinct Remark Codes
```{r, echo = FALSE,warning=FALSE,Include=FALSE,message=FALSE}
DBHYDRO_data_2021 %>%
group_by(REMARK_CODE) %>%
summarise(`Number of Records`=n(),`Percent of Total`=percent(n()/nrow(DBHYDRO_data_2021),accuracy = 0.01)) %>% kable() %>% kable_styling(bootstrap_options = c("striped", "hover")) 
```
...
###DQO Blank Contamination   (Parameters with % hits > 0.1)
```{r, echo = FALSE,warning=FALSE,Include=FALSE,message=FALSE}
